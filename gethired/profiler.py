...
def _bullet_lengths(bullets: tuple[Bullet, ...]) -> list[int]:
    return [len(bullet.text) for bullet in bullets]


def _opening_verbs(bullets: tuple[Bullet, ...]) -> tuple[str, ...]:
    counter: Counter[str] = Counter()
    for bullet in bullets:
        match = _OPENING_VERB_RE.match(bullet.text)
        if match:
            counter[match.group(1).lower()] += 1
    return tuple(verb for verb, _ in counter.most_common(10))


def _punctuation_density(bullets: tuple[Bullet, ...]) -> dict[str, float]:
    if not bullets:
...
def _sentence_counts(bullets: tuple[Bullet, ...]) -> tuple[int, int]:
    if not bullets:
        return (0, 0)
...
def build(master: MasterResume) -> VoiceProfile:
    ...
    lengths = _bullet_lengths(tuple(all_bullets))
    ...
    return VoiceProfile(
        ...
        opening_verbs=_opening_verbs(tuple(all_bullets)),
        punctuation_density=_punctuation_density(tuple(all_bullets)),
        sentence_count_per_bullet=_sentence_counts(tuple(all_bullets)),
    )
