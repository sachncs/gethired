...
def test_cover_letter_mirrors_master_contact(resume) -> None:
    ...
    assert resume.name in md, (
        f"signoff must include master name {resume.name!r}, got {md!r}"
    )


def test_cover_letter_opening_includes_role(resume) -> None:
...
def test_cover_letter_opening_includes_candidate_title(resume) -> None:
...
def test_cover_letter_body_mentions_seniority(resume) -> None:
...
def test_cover_letter_default_salutation_is_hiring_team(resume) -> None:
...
def test_cover_letter_sender_name_override(resume) -> None:
    """An explicit sender_name is used in the signoff instead of the master's name."""
    analysis = _sample_analysis()
    voice = build_profile(resume)
    result = compose(
        resume, analysis, voice, sender_name="Anonymous Applicant"
    )
    md = markdown(result.letter)
    assert "Anonymous Applicant" in md
    # But the master's name should NOT be there
    assert resume.name not in md


def test_cover_letter_markdown_contains_all_sections(resume) -> None:
...
def test_cover_letter_first_role_used_in_opening(resume) -> None:
    """The opening names the master's first experience role (most recent)."""
    analysis = _sample_analysis()
    voice = build_profile(resume)
    result = compose(resume, analysis, voice)
    opening = result.letter.paragraphs[0].text
    expected_role = resume.experience[0].role
    expected_company = resume.experience[0].company
